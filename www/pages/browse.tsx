...
const Home: NextPage<IHomeProps> = ({ meta, syllabiListings }) => {
  ...
  const updateRouterQueryParams = (filters: ISyllabiFilters) => {
    if (filters.academic_year != "")
      setCurrentQuery({ ...router.query, year: filters.academic_year });

    if (filters.academic_field != "")
      setCurrentQuery({ ...router.query, fields: filters.academic_field });

    if (filters.academic_level != "")
      setCurrentQuery({ ...router.query, levels: filters.academic_level });

    if (filters.language != "")
      setCurrentQuery({ ...router.query, languages: filters.language });

    if (filters.tags_include.length != 0)
      setCurrentQuery({ ...router.query, tags: filters.tags_include });
  };
