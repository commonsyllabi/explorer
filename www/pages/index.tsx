...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
  ...
  const syll_url = new URL(`syllabi/${process.env.FEATURED_SYLLABUS}`, process.env.API_URL);
  const syll_res = await fetch(syll_url)
  if (syll_res.ok) payload.syllabus = await syll_res.json()

  const coll_url = new URL(`collections/${process.env.FEATURED_COLLECTION}`, process.env.API_URL);
  const coll_res = await fetch(coll_url)
  if (coll_res.ok) payload.collection = await coll_res.json()

  const user_url = new URL(`users/${process.env.FEATURED_PROFILE}`, process.env.API_URL);
